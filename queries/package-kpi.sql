...
WITH    cteEID
          AS ( SELECT   execution_id
               ...
               WHERE    e.folder_name LIKE @folderNamePattern
                        ...
                        AND ( @executionId = -1
                              AND e.start_time >= DATEADD(HOUR, -@hourspan,
                                                          SYSDATETIME())
                            )
                        OR ( e.execution_id = @executionId )
             ),
        cteE
          AS ( SELECT   errors = COUNT(*)
        ...
        cteDW
          AS ( SELECT   duplicate_warnings = COUNT(*)
               FROM     [catalog].event_messages em
               WHERE    em.operation_id IN ( SELECT c.execution_id
                                             FROM   cteEID c )
                        AND em.event_name = 'OnWarning'
        ...
        cteMW
          AS ( SELECT   memory_warnings = COUNT(*)
               FROM     [catalog].event_messages em
               WHERE    em.operation_id IN ( SELECT c.execution_id
                                             FROM   cteEID c )
